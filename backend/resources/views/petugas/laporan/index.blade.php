...
@section('content')
    <style>
        @media print {
            body > div.flex.h-screen > div.w-64,
            body > div.flex.h-screen > div.flex-1 > header,
            .print-hidden {
            ...
            body > div.flex.h-screen > div.flex-1 {
                display: block !important;
                width: 100% !important;
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 mb-6 print-hidden">
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200">
        <div class="p-5 border-b border-gray-200 bg-gray-50 flex justify-between items-center print-hidden">
            ...
            <button type="button" onclick="window.print()"
                class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 text-sm font-semibold rounded-lg transition shadow-sm">
                Cetak / Print Laporan
            </button>
        </div>
                        ...
                        <tr class="hover:bg-gray-50 transition align-top">
                            ...
                            <td class="py-3 px-4 border-b">
                                <span class="px-2.5 py-1 rounded text-xs font-semibold
                                    {{ $peminjaman->status == 'selesai' ? 'bg-emerald-100 text-emerald-700' : '' }}
                                    {{ $peminjaman->status == 'dipinjam' ? 'bg-blue-100 text-blue-700' : '' }}
                                    {{ $peminjaman->status == 'telat' ? 'bg-red-100 text-red-700' : '' }}
                                    {{ $peminjaman->status == 'diajukan' ? 'bg-yellow-100 text-yellow-700' : '' }}">
